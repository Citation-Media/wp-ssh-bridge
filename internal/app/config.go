...
func writeConfigFile(path string, cfg Config, defaults Config) error {
	...
	writeStringValue(&body, "pull_destination", cfg.Destination, defaults.Destination)
	...
	writeStringValue(&body, "push_destination", cfg.PushDestination, defaults.PushDestination)
...
type RemoteTarget struct {
	// Destination is the whole SSH address in one value: an alias from ~/.ssh/config,
	// [user@]host[:port], or ssh://[user@]host[:port]. When set, User, Host, and Port
	// stay empty; the address carries them.
	Destination  string
	User         string
	Host         string
	Port         string
...
func (cfg Config) pushTarget() RemoteTarget {
	return RemoteTarget{
		Destination:  cfg.PushDestination,
		User:         cfg.PushUser,
		Host:         cfg.PushHost,
		Port:         cfg.PushPort,
...
func (target RemoteTarget) addressConfigured() bool {
...
func (cfg Config) validatePushRequired() error {
	return cfg.pushTarget().validate("push target", "set push_* config or pass --push-destination and --push-remote-path")
}
...
func (target RemoteTarget) validateValues(label string) error {
	if target.Destination != "" {
		if _, err := parseDestination(target.Destination); err != nil {
			return fmt.Errorf("%s: %w", label, err)
		}
		if target.User != "" || target.Host != "" || target.Port != "" {
			return fmt.Errorf("%s: the destination already carries the user, host, and port; remove the separate user, host, and port values or move them into the destination", label)
		}
	}
...
func (target RemoteTarget) configured() bool {
	return target.Destination != "" || target.User != "" || target.Host != "" || target.Port != "" || target.RemotePath != ""
}
...
func (cfg Config) envArgs() string {
	...
	add("WP_SSH_PULL_DESTINATION", cfg.Destination)
	...
	add("WP_SSH_PUSH_DESTINATION", cfg.PushDestination)
...
func mergeConfig(base Config, overlay Config) Config {
	...
	if overlay.PushDestination != "" {
		base.PushDestination = overlay.PushDestination
	}
	if overlay.PushUser != "" {
