...
Do not put private key contents in environment variables. SSH should use normal OpenSSH files, SSH config, or an agent. To inject the variables from a vault for one run, wrap the command: `op run --env-file=wp-ssh.env -- wp-ssh-bridge pull --silent` or `op run --environment <id> -- …` (1Password), `bws run --project-id <id> -- wp-ssh-bridge pull --silent` (Bitwarden Secrets Manager).
