...
```
   ...
   Add `--local-wp-path`, `--clone-images`, `--push-destination` with the other `--push-*` flags, and `--ssh-command` as needed. `init --silent` validates the values, writes the config, and in DDEV mode generates the provider files. With an alias as destination it prints what `ssh -G` resolves it to; if that line names the wrong user or host, the fix is in `~/.ssh/config`, not in the CLI config.
...
- Do not recommend copying private key material into config or env variables. SSH should use normal OpenSSH behavior, such as `~/.ssh/config`, loaded keys, or direct identity configuration outside this CLI. 1Password and Bitwarden work as ordinary SSH agents (`IdentityAgent` in `~/.ssh/config`, or `SSH_AUTH_SOCK`) and need nothing from the CLI; bastions and other wrappers go into `ssh_command` (`WP_SSH_SSH_COMMAND`, `--ssh-command`), for example `ssh -J bastion`, which is split on whitespace and cannot hold a path with spaces. Full guidance: https://wp-ssh-bridge.citation.media/docs/ssh-access.
...
- When values must come from a secrets manager, use env mode with its run command: `op run --env-file=wp-ssh.env -- wp-ssh-bridge pull --silent` (secret references `op://vault/item/field`) or `op run --environment <id>` for a 1Password Environment; `bws run --project-id <id> -- wp-ssh-bridge pull --silent` for Bitwarden Secrets Manager, with secrets named exactly like the `WP_SSH_*` variables and never `--no-inherit-env`, which drops the SSH agent socket. Only `clone_db_password` is a real secret; keep non-secret config in the file. Details: https://wp-ssh-bridge.citation.media/docs/secrets-managers.
