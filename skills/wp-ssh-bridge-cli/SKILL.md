...
- Do not recommend copying private key material into config or env variables. SSH should use normal OpenSSH behavior, such as `~/.ssh/config`, loaded keys, or direct identity configuration outside this CLI. 1Password and Bitwarden work as ordinary SSH agents (`IdentityAgent` in `~/.ssh/config`, or `SSH_AUTH_SOCK`) and need nothing from the CLI. Bastions, proxy commands, and a pinned key (`IdentityFile <public key>` plus `IdentitiesOnly yes`, the fix for `Too many authentication failures`) belong in a `~/.ssh/config` `Host` block used as the destination; the CLI deliberately has no option for them. Full guidance: https://wp-ssh-bridge.citation.media/docs/ssh-access.
