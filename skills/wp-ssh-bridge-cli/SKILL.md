...
- For clone-style pulls (host-to-host site migrations), read `references/clone.md` before recommending commands. A first push into an empty target stops with `the push target URL is unknown` until `push_url`, `WP_SSH_PUSH_URL`, `--push-url`, a push domain mapping, or `--skip-search-replace` is given. Recommend `wp-ssh-bridge clone`; do not recommend `pull --clone`, `push --clone`, or the former `migrate` command name.
...
- A pull wraps the local database import in WordPress maintenance mode, and a push does the same on the target for the whole run; both lift it again even on failure. Maintenance mode is skipped automatically when the destination has no installed WordPress site (`wp core is-installed` fails, for example an empty directory or a new database) and for `clone --clean-target`; the CLI prints `Skipping ... maintenance mode`, which is expected. Recommend `--skip-maintenance-mode` only when the user asks for it or WP-CLI cannot toggle it on that side. `Enabling local maintenance mode failed` with `The site you have requested is not installed` indicates an older CLI that needs updating.
...
- Hosts that disable PHP `exec()` for the command line (verified on Hostinger) make `wp db export` exit with status 255 and no message. Pull preflight reports `PHP function compatibility enabled (exec allowed for WP-CLI db export only)` and runs only the export with `php -d disable_functions=<host list without the needed functions>`; nothing on the host changes. A fatal error containing `does not allow re-enabling them` means the host blocks even that override: ask the provider to allow the named functions for PHP CLI. `PHP function check failed ... could not determine the PHP that runs WP-CLI` means `wp cli info` failed and `wp` is a wrapper script; have the user run `wp cli info` on the host. A silent exit 255 during export indicates an older CLI that needs updating. Details: https://wp-ssh-bridge.citation.media/docs/troubleshooting/disabled-php-functions.
- Each run authenticates once per SSH login: preflight opens a shared OpenSSH connection that later commands and transfers reuse, closed on exit, failure, or Ctrl+C. Repeated agent approval prompts within one run indicate an older CLI. DDEV provider steps are separate processes and authenticate once each.
