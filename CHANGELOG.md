...
- Document secrets managers: which values belong in a vault, and how `op run` with secret references or a 1Password Environment, `bws run` with a Secrets Manager project, and the matching GitHub Actions inject them as `WP_SSH_*` variables for one run, locally, under DDEV, and in CI.
